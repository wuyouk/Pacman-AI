...
def enhancedFeatureExtractorDigit(datum):
    ...
    after trying different features like symmetry, pixels distributions
    found that dividing the datum into 9 (3*3) areas, and extract features based on the pixel in each area
    tune the threshold by printing out the training datum and looking for dividing point to increase the test performance
    ...
    for i in range(9):
        areaList.append(0)
    for i in range(9):
        for x in range((DIGIT_DATUM_WIDTH-1)/3):
            x += int(i%3)*(DIGIT_DATUM_WIDTH-1)/3
            for y in range((DIGIT_DATUM_HEIGHT-1)/3):
                y += int(i/3)*(DIGIT_DATUM_HEIGHT-1)/3
                if datum.getPixel(x,y) > 0:
                    areaList[i] += 1
    'maximum num of pixels in each area = 81'
    '''print 'area0: ', areaList[0]/81.0
    print 'area1: ', areaList[1]/81.0
    print 'area8: ', areaList[8]/81.0'''
    for i in range(2):
        if areaList[0]/81.0 > 0:
            features[100+i*10] = 1
        else:
            features[100+i*10] = 0
    for i in range(2):
        if areaList[1]/81.0 > 0.23:
            features[100+i*10+1] = 1
        else:
            features[100+i*10+1] = 0
    for i in range(2):
        if areaList[2]/81.0 > 0.13:
            features[100+i*10+2] = 1
        else:
            features[100+i*10+2] = 0
    for i in range(1):
        if areaList[3]/81.0 > 0.13:
            features[100+i*10+3] = 1
        else:
            features[100+i*10+3] = 0
    for i in range(2):
        if areaList[4]/81.0 > 0.5:
            features[100+i*10+3+1] = 1
        else:
            features[100+i*10+3+1] = 0
    for i in range(1):
        if areaList[5]/81.0 > 0.39:
            features[100+i*10+5] = 1
        else:
            features[100+i*10+5] = 0
    for i in range(2):
        if areaList[6]/81.0 > 0.05:
            features[100+i*10+6] = 1
        else:
            features[100+i*10+6] = 0
    for i in range(2):
        if areaList[7]/81.0 > 0.55:
            features[100+i*10+7] = 1
        else:
            features[100+i*10+7] = 0
    '''for i in range(1):
        if areaList[8]/81.0 > 0.18:
            features[100+i*10+8] = 1
        else:
            features[100+i*10+8] = 0'''

    '''for a in range(9):
        for i in range(5):
            if areaList[a]/81.0 > 0.5:
                features[100+i*10+a] = 1
            else:
                features[100+i*10+a] = 0'''
    ...
    for x in range(DIGIT_DATUM_WIDTH):
        for y in range(DIGIT_DATUM_HEIGHT/2):
            if datum.getPixel(x,y) == datum.getPixel(x,27-y):
                symmetryV += 1
    for y in range(DIGIT_DATUM_HEIGHT):
        for x in range(DIGIT_DATUM_WIDTH/2):
            if datum.getPixel(x,y) == datum.getPixel(27-x,y):
                symmetryH += 1
    'maximum symmetry value = 392'

    'print symmetryH'
    for i in range(5):
        if symmetryH/392.0 > 0.82:
            features[i*10+0] = 1
        else:
            features[i*10+0] = 0
        if symmetryV/392.0 > 0.85:
            features[i*10+1] = 1
        else:
            features[i*10+1] = 0
    'print symmetryV'
    ...
    for y in range(DIGIT_DATUM_HEIGHT/2):
        ...
        for x in range(DIGIT_DATUM_WIDTH):
            if datum.getPixel(x,y) > 0:
                upP += 1
    'maximum position distribution value = 392'
    print 'leftP is', leftP
    for i in range(5):
        if leftP/392.0 > 0.16:
            features[i*10+2] = 1
        else:
    ...
    for x in range(DIGIT_DATUM_WIDTH):
        for y in range(DIGIT_DATUM_HEIGHT):
            if datum.getPixel(x,y) > 0:
                numP += 1
    'maximum number of on pixels = 784'

    rightP = numP - leftP
    upP = numP - downP
    for i in range(5):
        if rightP/392.0 < 0.27:
            features[i*10+8] = 1
        else:
            features[i*10+8] = 0
        if upP/392.0 > 0.35:
    ...
    leftMore = 0
    downMore = 0
    'print leftP/float(numP)'
    for i in range(5):
        if leftP/float(numP) > 0.5:
            leftMore = 1
        else:
            leftMore = 0
        features[i*10+4] = leftMore
        if downP/float(numP) > 0.5:
            downMore = 1
        else:
            downMore = 0
        features[i*10+5] = downMore
    'print downP/float(numP)'
    ...
    '''
    print 'datum new features:'
    for i in range(7):
        print features[i]
        '''
    

    '''print 'a datum finished'''
...
def enhancedPacmanFeatures(state, action):
    ...
    for foodPos in foodList:
        disFoodList.append(util.manhattanDistance(pacPos, foodPos))
    features['minFoodDis'] = 0
    if not disFoodList == []:
        minFoodDis = min(disFoodList)
        if not minFoodDis == 0:
            features['minFoodDis'] = 23.0/minFoodDis

    numFood = successor.getNumFood()
    features['numFood'] = numFood*1.0

    ghostPosS = successor.getGhostPositions()
    disGhostList = []
    for ghostPos in ghostPosS:
        disGhostList.append(util.manhattanDistance(pacPos, ghostPos))
    features['minGhostDis'] = 0
    if not disGhostList == []:
        minGhostDis = min(disGhostList)
        if not minGhostDis == 0:
            features['minGhostDis'] = minGhostDis/2.0

    '''avgGhostDis = sum(disGhostList)/len(disGhostList)
    features['avgGhostDis'] = avgGhostDis/13'''

    capsulePosS = successor.getCapsules()
    disCapsuleList = []
    for capsulePos in capsulePosS:
        disCapsuleList.append(util.manhattanDistance(pacPos, capsulePos))
    features['minCapsuleDis'] = 0
    if not disCapsuleList == []:
        minCapsuleDis = min(disCapsuleList)
        if not minCapsuleDis == 0:
            features['minCapsuleDis'] = 3/minCapsuleDis
    ...
    for ghostState in ghostStates:
        scaredTime.append(ghostState.scaredTimer)
    features['scaredTime'] = 0
    features['scaredTime'] = sum(scaredTime)/len(scaredTime)/5.0
    return features
    '''
