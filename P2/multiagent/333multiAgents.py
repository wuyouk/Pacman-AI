...
class ReflexAgent(Agent):
    ...
    def evaluationFunction(self, currentGameState, action):
        ...
        for food in FoodList:
            if newFood[food[0]][food[1]]==True:
                DF+=abs(newPos[0]-food[0])+abs(newPos[1]-food[1])
...
class MinimaxAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        def pacmanMax(gameState,depth):
            if gameState.isWin() or gameState.isLose():
                return gameState.getScore()
            maxV=float("-inf")
            for a in gameState.getLegalActions(0):
                suV=ghostMin(gameState.generateSuccessor(0, a), depth, 1)
                if maxV<suV:
                    maxV=suV
                    maxA=a
            if depth==1:
        ...
        def ghostMin(gameState,depth,gIndex):
            if gameState.isWin() or gameState.isLose():
                return gameState.getScore()
            ...
            for a in gameState.getLegalActions(gIndex):
                if nextAgent==0:
                    if depth==self.depth:
                        ...
                        v=pacmanMax(gameState.generateSuccessor(gIndex, a), depth + 1)
                else:
                    v=ghostMin(gameState.generateSuccessor(gIndex, a), depth, nextAgent)
                if v<minV:
                    minV=v
            return minV

        return pacmanMax(gameState,1)
