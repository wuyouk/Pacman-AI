...
class ExactInference(InferenceModule):
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        """
        "*** YOUR CODE HERE ***"
        'tempBeliefs = self.beliefs.copy()'
        tempBeliefs = util.Counter()
        for p in self.legalPositions:
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, p))
            for newPos, prob in newPosDist.items():
                tempBeliefs[newPos] += prob * self.beliefs[p]
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        while numParticles > 0:
            parList.append(self.legalPositions[i%numPos])
            i += 1
            numParticles -= 1
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance == None:
            ...
            for i in range(len(self.particles)):
                newParList.append(self.getJailPosition())
                self.particles = newParList
        else:
            updateWeight = util.Counter()
            for par in self.particles:
                updateWeight[par] += emissionModel[util.manhattanDistance(pacmanPosition, par)]
            'if all(val==0 for val in updateDistribution.values()):'
            if updateWeight.totalCount() == 0:
                self.initializeUniformly(gameState)
            else:
                parList = []
                for i in range(len(self.particles)):
                    parList.append(util.sample(updateWeight))
                self.particles = parList
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        "*** YOUR CODE HERE ***"
        parList = []
        ghostsPosPermutation = list(itertools.product(self.legalPositions, repeat=self.numGhosts))
        random.shuffle(ghostsPosPermutation)
        
        '''for i in range(self.numParticles):
            parList.append(random.choice(ghostsPosPermutation))
        self.particles = parList'''
    
    
        numPos = len(ghostsPosPermutation)
        numParticles = self.numParticles
        i = 0
        while numParticles > 0:
            parList.append(ghostsPosPermutation[i%numPos])
            i += 1
            numParticles -= 1
        self.particles = parList
        ...
        for particle in self.particles:
            ...
            for i in range(self.numGhosts):
                if noisyDistances[i] == None:
                    ...
                    particleL = list(particle)
                    weight *= emissionModels[i][util.manhattanDistance(pacmanPosition, particleL[i])]
            'updateWeight[particle] = weight'
            updateWeight[particle] += weight
        if updateWeight.totalCount() == 0:
            self.initializeParticles()
            for i in range(self.numGhosts):
